...
import {
    convertFtbQuests,
    QuestInputFileSystem,
    QuestOutputFileSystem,
} from "odysseus";
...
async function handleInteraction(
    interaction: APIPingInteraction | APIDMInteraction | APIGuildInteraction,
    respond: (response: APIInteractionResponse) => void,
) {
    ...
    const getOption = <T extends APIApplicationCommandInteractionDataOption>(
        name: string,
    ) => options?.find((option): option is T => option.name === name);

    const questDataOption =
        getOption<APIApplicationCommandInteractionDataAttachmentOption>(
            "quest-file",
        );
    const typeOption =
        getOption<APIApplicationCommandInteractionDataStringOption>("type");
    const questDataAttachment = questDataOption
        ? interaction.data.resolved?.attachments?.[questDataOption.value]
        : undefined;
    ...
    const buffer = await axios
        .get(questDataAttachment.url, { responseType: "arraybuffer" })
    ...
    const inputFileSystem: QuestInputFileSystem = {
        ...
        readDirectory(name: string) {
            ...
            folder.forEach((name, file) =>
                fileContents.push(
                    file.async("text").then((data) => [data, name]),
                ),
            );
            ...
            files: [
                {
                    name: "quests.zip",
                    data: Buffer.from(
                        await outputZip.generateAsync({ type: "binarystring" }),
                        "binary",
                    ),
...
app.post(
    "/",
    async (
        request: Request<object, unknown, string, APIInteractionResponse>,
        response,
    ) => {
        ...
        const interaction:
            | APIPingInteraction
            | APIDMInteraction
            | APIGuildInteraction = JSON.parse(request.body);
