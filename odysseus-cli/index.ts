...
for (let i = 0; i < args.length; ++i) {
    ...
    if (equals >= 0) {
        ...
        if (!nextArgument) {
            throw new Error(
                "Invalid argument list, no value provided for " + argumentType,
            );
        }
...
if (!type) {
    if (fs.statSync(inputPath).isDirectory()) {
        ...
        throw new Error(
            "No type specified and couldn't infer from input, use --type or -t",
        );
    }
}
        ...
        return doesExist
            ? Promise.all(
                  (await readDir(path)).map(async (file) => [
                      (await readFile(`${path}/${file}`)).toString(),
                      file,
                  ]),
              )
            : [];
