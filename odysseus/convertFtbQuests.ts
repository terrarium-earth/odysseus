    ...
    (
        ...
        | ({
              ...
              | {
                    observe_type:
                        | ObserveType.BLOCK
                        | ObserveType.BLOCK_ENTITY_TYPE
                        | ObserveType.ENTITY_TYPE;
                    to_observe: ResourceLocation;
                }
              | {
                    observe_type:
                        | ObserveType.BLOCK_TAG
                        | ObserveType.ENTITY_TYPE_TAG;
                    to_observe: TagKey;
                }
              | {
                    observe_type:
                        | ObserveType.BLOCK_STATE
                        | ObserveType.BLOCK_ENTITY;
                    to_observe: string;
                }
          ))
        | {
...
type QuestReward = BasicQuestObject &
    (
        | Advancement
        | {
              type: FtbId<"choice">;
...
function convertItemReward(reward: FtbItem): HeraclesQuestReward {
    const item =
        typeof reward.item === "object" ? reward.item : { id: reward.item };
    return {
        type: "heracles:item",
        item: {
...
function toObject<T extends QuestObject, R extends object>(
    array: T[],
    warnings: Set<string>,
    convertor: (value: T) => R | null,
): Record<string, R> {
    return array
        .map((value) => {
            try {
        ...
        .filter((value): value is { id: string; data: R } => Boolean(value))
        .reduce(
            (existing, current) => ({
                ...existing,
...
function inferTitle(t: HeraclesQuestTask): string | undefined {
    switch (t.type) {
        case "heracles:item":
            return `Acquire: ${t.item}`;
        case "heracles:item_interaction":
            return `Interact: ${t.item}`;
        case "heracles:item_use":
            return `Use: ${t.item}`;
        case "heracles:stat":
            return `Increase ${t.stat}`;
        case "heracles:changed_dimension":
            return `Visit ${t.to}`;
        case "heracles:advancement":
            return `Complete ${
                t.advancements.length === 0 ? "Advancement" : t.advancements[0]
            }`;
        case "heracles:structure":
            return `Find ${
                t.structures.length === 0 ? "Structure" : t.structures[0]
            }`;
        case "heracles:biome":
            return `Visit ${t.biomes.length === 0 ? "Biome" : t.biomes[0]}`;
        case "heracles:check":
            return `Check this Task`;
        case "heracles:block_interaction":
            return `Interact ${t.block}`;
        case "heracles:entity_interaction":
            return `Interact: ${t.entity}`;
        case "heracles:kill_entity":
            return `Kill: ${t.entity}`;
        case "heracles:location":
            return `Visit a Location`;
        case "heracles:recipe":
            return `Craft a Recipe`;
        case "heracles:xp":
            return `Acquire XP`;
...
function inferIcon(t: HeraclesQuestTask): HeraclesQuestIcon | undefined {
    switch (t.type) {
        case "heracles:item":
            return t.item.startsWith("#") ? undefined : convertIcon(t.item);
        case "heracles:item_interaction":
            return t.item.startsWith("#") ? undefined : convertIcon(t.item);
        case "heracles:item_use":
            return t.item.startsWith("#") ? undefined : convertIcon(t.item);
        case "heracles:stat":
            return convertIcon("minecraft:spyglass");
        case "heracles:changed_dimension":
            return convertIcon("minecraft:netherrack");
        case "heracles:advancement":
            return convertIcon("minecraft:knowledge_book");
        case "heracles:structure":
            return convertIcon("minecraft:structure_block");
        case "heracles:biome":
            return convertIcon("minecraft:birch_sapling");
        case "heracles:check":
            return convertIcon("minecraft:green_wool");
        case "heracles:block_interaction":
            return t.block.startsWith("#") ? undefined : convertIcon(t.block);
        case "heracles:entity_interaction":
            return t.entity.startsWith("#")
                ? undefined
                : convertIcon((t.entity + "_spawn_egg") as RegistryValue);
        case "heracles:kill_entity":
            return t.entity.type.startsWith("#")
                ? undefined
                : convertIcon((t.entity.type + "_spawn_egg") as RegistryValue);
        case "heracles:location":
            return convertIcon("minecraft:compass");
        case "heracles:recipe":
            return convertIcon("minecraft:crafting_table");
        case "heracles:xp":
            return convertIcon("minecraft:experience_bottle");
        case "heracles:composite":
            return undefined;
        case "heracles:dummy":
            return undefined;
...
export const convertFtbQuests = async (
    input: QuestInputFileSystem,
    output: QuestOutputFileSystem,
) => {
    ...
    const [questFile, groups, chapters, rewardTables] = await Promise.all([
        readSNbtFile("data.snbt").then((data) => data as QuestFile),
        readSNbtFile("chapter_groups.snbt").then(
            (groups) => (groups as ChapterGroups).chapter_groups,
        ),
        readDirectory("chapters").then((chapters) =>
            sortOrdered(chapters as (Chapter & OrderIndex)[]),
        ),
        readDirectory("reward_tables").then((tables) =>
            sortOrdered(tables as (RewardTable & OrderIndex)[]),
        ),
    ]);
    ...
    for (const group of [...groups, null]) {
        ...
        for (const chapter of groupedChapters[group?.id ?? ""] ?? []) {
            ...
            for (const quest of chapter.quests) {
                const tasks = toObject(quest.tasks ?? [], warnings, (task) =>
                    convertTask(task, questFile),
                );
                const rewards = toObject(
                    quest.rewards ?? [],
                    warnings,
                    (reward) => convertReward(reward, rewardTables),
                );
                ...
                const questTitle = quest.title
                    ? formatString(quest.title)
                    : inferredData?.title;
                const questSubtitle = quest.subtitle
                    ? formatString(quest.subtitle)
                    : undefined;
                const questIcon = quest.icon
                    ? convertIcon(quest.icon)
                    : inferredData?.icon;
                let hidden:
                    | "LOCKED"
                    | "IN_PROGRESS"
                ...
                const iconBackground = iconBackgroundTexture(
                    quest.shape ??
                        chapter.default_quest_shape ??
                        questFile.default_quest_shape,
                );
                        ...
                        description: escapeFormatters([
                            ...
                            ...(taskIds.length
                                ? [
                                      ...taskIds.map(
                                          (taskId) =>
                                              `<task task="${taskId}" quest="${quest.id}"/>`,
                                      ),
                                      "<hr/>",
                                  ]
                                : []),
                            ...
                            ...(rewardsIds.length
                                ? [
                                      ...
                                      ...rewardsIds.map(
                                          (rewardId) =>
                                              `<reward reward="${rewardId}" quest="${quest.id}"/>`,
                                      ),
                                  ]
                                : []),
                        ]),
                ...
                const groupPart = "";
                const chapterPart = chapterTitle
                    ?.toLowerCase()
                    .replaceAll(/[^a-z0-9]/g, "");
                ...
                fileWrites.push(
                    output.writeFile(
                        `quests/${groupPart}${
                            chapterPart?.length ? chapterPart : chapter.title
                        }/${quest.id}.json`,
...
function convertTask(task: QuestTask, questFile: QuestFile): HeraclesQuestTask {
    ...
    switch (task.type) {
        ...
        case "ftbquests:item":
        case "item": {
            let collectionType: "AUTOMATIC" | "MANUAL" | "CONSUME" | undefined =
                undefined;

            if (
                task.consume_items === true ||
                questFile.default_consume_items === true
            ) {
        ...
        case "observation": {
            switch (task.observe_type) {
                ...
                case ObserveType.BLOCK_ENTITY: {
                    ...
                    const block = task.to_observe
                        .substring(0, stateStart)
                        .trim() as ResourceLocation;

                    const nbtStart = task.to_observe.indexOf("{");
                    const stateString = task.to_observe.substring(
                        stateStart + 1,
                        stateEnd,
                    );
                    const nbtString =
                        nbtStart >= 0
                            ? task.to_observe.substring(
                                  nbtStart + 1,
                                  task.to_observe.lastIndexOf("}"),
                              )
                            : null;

                    const state: Record<string, string> = stateString
                        .split(",")
                        .map((property) => property.split("="))
                        .reduce(
                            (existingState, property) => ({
                                ...existingState,
        ...
        default:
            throw new ConversionError(
                `Don't know how to convert task of type ${task.type}.`,
            );
    }
}

function convertReward(
    reward: QuestReward,
    rewardTables: (RewardTable & OrderIndex)[],
): HeraclesQuestReward | null {
    const rewardBase = {
        title: reward.title,
    ...
    switch (reward.type) {
        ...
        case "loot": {
            ...
            if ("table" in reward) {
                rewardTable = rewardTables.find(
                    (table) => table.order_index === reward.table,
                );
            } else if ("table_id" in reward) {
                rewardTable = rewardTables.find(
                    (table) => table.id === reward.table_id,
                );
            }
            ...
            if (rewardTable.loot_table_id) {
                ...
                throw new ConversionError(
                    `Don't know how to convert reward ${reward}`,
                );
            }
        }
        case "choice": {
            ...
            if (rewardTable.rewards) {
                const rewards: Record<string, HeraclesQuestReward> = {};
                rewardTable.rewards.forEach((tableReward, i) => {
                    rewards[reward.id + "_" + i] =
                        convertItemReward(tableReward);
                });
                return {
        ...
        default:
            throw new ConversionError(
                `Don't know how to convert reward of type ${reward.type}.`,
            );
    }
}
